...
class TestMeshAxis(unittest.TestCase):
    ...
    def test_isorthogonal_rejects_non_radix_unique_layout(self) -> None:
        # These axes are pairwise radix-separable, and all sums from
        # {0, 5} + {0, 6} + {0, 7} are distinct. The tightened orthogonality
        # contract still rejects the triple because stride 7 lands inside the
        # 0..11 span generated by strides 5 and 6.
        a = MeshAxis.of(2, 5)
        b = MeshAxis.of(2, 6)
        c = MeshAxis.of(2, 7)
        self.assertTrue(a.isorthogonal(b))
        self.assertTrue(a.isorthogonal(c))
        self.assertTrue(b.isorthogonal(c))
        self.assertFalse(a.isorthogonal(b, c))

    def test_isorthogonal_non_nested_overlapping(self) -> None:
        # Non-radix strides with a collision: 0 + 2 == 2 + 0.
        a = MeshAxis.of(3, 1)  # ranks {0, 1, 2}
        b = MeshAxis.of(3, 2)  # ranks {0, 2, 4}
        self.assertFalse(a.isorthogonal(b))

    def test_isorthogonal_large_world_size_does_not_materialize_ranks(self) -> None:
        # Large logical mesh: 4 * 64 * 48 = 12288 ranks.
        tp = MeshAxis.of(4, 1)
        ep = MeshAxis.of(64, 4)
        dp = MeshAxis.of(48, 256)

        with mock.patch.object(
            type(tp.layout),
            "all_ranks_from_zero",
            side_effect=AssertionError("isorthogonal should not enumerate ranks"),
        ):
            self.assertTrue(tp.isorthogonal(ep, dp))
